...
TriplePatternFragmentsRdfView.prototype._generateRdf = function (settings, data, metadata, done) {
  ...
  settings.resultStream.on('metadata', function (meta) {
    ...
    if (meta.subjectsPerObjectCount)
      metadata(fragment.pageUrl, stats + 'subjectsPerObjectCount', '"' + meta.subjectsPerObjectCount + '"^^' + xsd + 'double');
    if (meta.objectsPerSubjectCount)
      metadata(fragment.pageUrl, stats + 'objectsPerSubjectCount', '"' + meta.objectsPerSubjectCount + '"^^' + xsd + 'double');
