...
// Helper functions
var SUBJECT_COUNT_REGEX = /^subjectsPerObjectCount/;
var OBJECT_COUNT_REGEX = /^objectsPerSubjectCount/;

function isCountMetadata(key) {
  return key.match(SUBJECT_COUNT_REGEX) || key.match(OBJECT_COUNT_REGEX);
}
...
TurtleWriter.prototype.writeFragment = function (destination, tripleStream, settings) {
  ...
  tripleStream.on('metadata', function (metadata) {
    ...
    Object.keys(metadata).forEach(function (key) {
      isCountMetadata(key) && writer.meta(fragment.pageUrl, stats + key, '"' + metadata[key] + '"^^' + xsd + 'double');
    });
