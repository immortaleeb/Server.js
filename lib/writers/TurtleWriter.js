...
TurtleWriter.prototype.writeFragment = function (destination, tripleStream, settings) {
  ...
  tripleStream.on('metadata', function (metadata) {
    ...
    // Other metadata
    if (metadata.subjectsPerObjectCount)
      writer.meta(fragment.pageUrl, stats + 'subjectsPerObjectCount', '"' + metadata.subjectsPerObjectCount + '"^^' + xsd + 'double');
    if (metadata.objectsPerSubjectCount)
      writer.meta(fragment.pageUrl, stats + 'objectsPerSubjectCount', '"' + metadata.objectsPerSubjectCount + '"^^' + xsd + 'double');
